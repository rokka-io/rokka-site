...
function getDocumentScrollTop() {
  return window.scrollY || Math.max(document.documentElement.scrollTop, document.body.scrollTop) || window.pageYOffset || 0;
}
...
function offsetTop(element) {
  return element.getBoundingClientRect().top + getDocumentScrollTop() - (document.documentElement.clientTop || document.body.clientTop);
}
...
var _domDelegate = __webpack_require__(2);
...
var _easeComponent = __webpack_require__(3);
...
var Accordion = function () {
  ...
  _createClass(Accordion, [{
    ...
    key: '_scrollToActive',
    value: function _scrollToActive() {
      var _this = this;

      // Wait for the animation to complete
      setTimeout(function () {
        var top = _this._current.getBoundingClientRect().top;

        if (top <= 60) {
          _scroll2.default.top((0, _scrollDoc2.default)(), (0, _utils.offsetTop)(_this._current) - 74, { ease: _easeComponent.inOutSine });
        }
      }, 500);
    }
...
var DESKTOP_MIN_HEIGHT = 80;
var DESKTOP_MAX_HEIGHT = 160;
var MOBILE_HEIGHT = 61;
...
var Navbar = function () {
  ...
  _createClass(Navbar, [{
    ...
    value: function _handleScroll() {
      var scrollY = (0, _utils.getDocumentScrollTop)();
      var animationBase = Math.max(Math.min(1, (scrollY - this._threshold + this._maxHeight) / this._minHeight), 0);
      var logoScale = 1 - animationBase / 2;
      var navbarHeight = Math.max(Math.min(this._maxHeight, this._threshold - scrollY), 61);

      if ((0, _media2.default)('md')) {
    ...
    value: function _setSizes() {
      ...
      switch (this._options.mode) {
        case 'window':
          ...
          break;
        case 'section':
          this._threshold = document.querySelector('.section-spacer').clientHeight;
          break;
        default:
...
var DISABLED_BTN_CLASS = 'btn--disabled';
...
var Reel = function () {
  ...
  function Reel(el) {
    ...
    this._children = [].concat(_toConsumableArray(this._el.querySelectorAll('.reel__item')));
  ...
  _createClass(Reel, [{
    ...
    value: function _addEventListeners() {
      ...
      this._touchManager = new _hammerjs2.default.Manager(this._rollNode, {
    ...
    key: '_handlePanEnd',
    value: function _handlePanEnd(e) {
    ...
    key: '_handleNavigationClick',
    value: function _handleNavigationClick(e) {
      var operator = e.currentTarget === this._nextBtnNode ? '-' : '+';
      var destination = _utils.operators[operator](this._lastPosition, this._reelWidth);
    ...
    value: function _setSizes() {
      ...
      this._rollWidth = parseInt(this._children.reduce(function (prev, item) {
        return prev + item.getBoundingClientRect().width;
      }, 0).toFixed(), 10);
      this._maxPosition = -1 * parseInt((this._rollWidth - this._reelWidth).toFixed(), 10);

      var scopedPosition = this._getMinMaxPosition(this._position);
    ...
    value: function _updateNavigation() {
      var position = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this._position;
    ...
    key: '_startAnimation',
    value: function _startAnimation(destination) {
      var duration = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 700;
      var easeFn = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : _easeComponent.inOutSine;
    ...
    key: '_drift',
    value: function _drift(velocity) {
      var extra = 300 * velocity;
      var destination = this._getMinMaxPosition(this._lastPosition + extra);
      var duration = 350 * Math.max(1, Math.abs(velocity));
      ...
      this._startAnimation(destination, duration, _easeComponent.outExpo);
    }
    ...
    value: function _draw() {
      ...
      var _animation = this._animation,
          ...
          startX = _animation.startX,
          destination = _animation.destination,
          easeFn = _animation.easeFn;
      ...
      var easing = easeFn(progress);
      var position = startX + (destination - startX) * easing;
    ...
    key: '_getMinMaxPosition',
    value: function _getMinMaxPosition(position) {
    ...
    value: function _setPosition(position) {
      position = parseInt(position.toFixed(), 10);
      var transform = 'translateX(' + position + 'px)';
  ...
  _createClass(Weather, [{
      ...
      return fetch(ENDPOINT + '?id=' + this._cities.join(',') + '&units=metric&appid=' + this._appId).then(function (response) {
        ...
        if (list) {
          list.forEach(function (item) {
            _this._data[item.id] = item;
          });
        }
      }).then(this._updateIcons.bind(this)).catch(this._updateIcons.bind(this));
    }
  }, {
    key: '_updateIcons',
    value: function _updateIcons() {
      ...
      this._elements.forEach(function (el) {
        ...
        var weather = _this2._getWeather(id);

        if (weather) {
          el.setAttribute('title', weather.main);
        }
        (0, _ui.updateIcon)(iconEl, _this2._getIcon(id));
      });
    }
  }, {
    key: '_getWeather',
...
var _reel = __webpack_require__(8);
