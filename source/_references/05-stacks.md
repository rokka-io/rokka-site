...
| avif.quality | 45 | 1 | 100 | AVIF quality setting, lower number means smaller file size and worse lossy quality.                                                                                                                                                                                               |
| jxl.quality | 70 | 1 | 100 | JPEG XL quality setting, lower number means smaller file size and worse lossy quality.                                                                                                                                                                                            |
| gif.quality | 70 | 1 | 100 | Gif quality setting in the [optimization phase](#additional-image-optimizations). Choose a setting of 100 for lossless quality. Lower values also means lower file size. See the [rendering animated GIFs chapter](./render.html#rendering-animated-gifs).                        |
| jp2.quality | 40 | 1 | 100 | JPEG 2000 quality setting, lower number means smaller file size and worse lossy quality.                                                                                                                                                                                          |
| av1.quality | 70 | 1 | 100 | AV1 video quality setting, lower number means smaller file size and worse lossy quality. See the [video chapter](videos.html).                                                                                                                                                    |
| webm.quality | 50 | 1 | 100 | WebM video quality setting, lower number means smaller file size and worse lossy quality. See the [video chapter](videos.html).                                                                                                                                                   |
| mp4.quality | 50 | 1 | 100 | MP4 video quality setting, lower number means smaller file size and worse lossy quality. See the [video chapter](videos.html).                                                                                                                                                    |
...
| autoformat.include | null | - | - | You can opt some formats into autoformat. Currently only `avif` is supported. See the [autoformat chapter](#autoformat) below.                                                                                                                                                    |
...
| dpi | - | 10 | 5000 | Sets some dpi metadata info to an image, but doesn't resize them. It only works for PNG and TIFF (but not JPEG), currently, and you do have to set `optim.disable_all` to true. DPI doesn't matter in the web context, usually you don't need this.                               |
...
| pdf.page | 1 | 1 | 1000 | Which page of a PDF to render. Defaults to the first page.                                                                                                                                                                                                                       |
| pdf.pages | - | - | - | Which pages of a PDF to render, as a comma separated list of page numbers and `..` ranges (`1,3,5..7`), or `all`. Only works on the [render without storing](render.html#rendering-pdfs-and-several-pages-at-once) endpoint, where it returns a ZIP with one image per page; at most 50 pages. On a stored PDF use `pdf.page` instead. Note `..` and not `-`, which separates stack options in the URL. |
| min_video_width | - | 0 | 6000 | Minimum width a generated video should have. See the [video chapter](videos.html).                                                                                                                                                                                               |
| min_video_height | - | 0 | 2200 | Minimum height a generated video should have. See the [video chapter](videos.html).                                                                                                                                                                                              |
...
The invalidation is processed **asynchronously**: a `200` response (with `"status": "ok"` in the body) means the paths were accepted for invalidation, not that the CDN cache is already cleared. It usually completes within a few seconds.
...
| image.entropy | Returns the entropy (a measure of "busyness") of the current source image. |
| image.subject_area.* | Returns subject area data for width, height, x and y.  |
| image.hasDetectionFace | Returns true, if the source image has [face detection](dynamic-metadata.html) data. |
| image.face_detection.* | Returns face detection subject area data for width, height, x and y. `image.detection_face.*` is an alias for the same data. |
| image.meta_user.* | Returns a [user metadata](user-metadata.html) field, eg. `image.meta_user.foo`. |
...
| avif | The image will never be converted to AVIF, even if the browser would understand it. |
